...
public class Anagram {
    ...
    public static int[] countAlphabet(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            int asciiChar = (int) s.charAt(i);
            if(asciiChar <= 90 && asciiChar >= 65) {
                alpha[asciiChar-65] += 1;
            } else if(asciiChar <= 122 && asciiChar >= 97) {
                alpha[asciiChar-97] += 1;
            }
        }
    ...
    public static boolean isAnagram(String s1,
        ...
        int[] alpha1 = countAlphabet(s1);
        int[] alpha2 = countAlphabet(s2);
        for (int i = 0; i < alpha1.length; i++) {
            if(alpha1[i] != alpha2[i]) {anagram = false;}
        }
        return anagram;
    }
}
