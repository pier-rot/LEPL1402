...
public class FuturesLibrary {
    ...
    public static int countMatchingBooks(Library library,
        ...
        if (countBooks < 0 || startBookIndex < 0 || countBooks > library.getNumberOfBooks()) {
    ...
    public static int countMatchingBooksWithThreads(Library library,
        ...
        int blockSize = (library.getNumberOfBooks() % countThreads == 0 ? library.getNumberOfBooks() / countThreads : library.getNumberOfBooks() / countThreads + 1);
        ...
        for (int i = 0; i < countThreads; i++) {
            ...
            int block = Math.min(blockSize, library.getNumberOfBooks() - startIndex);

            futures.add(executor.submit(() -> countMatchingBooks(library, predicate, startIndex, block)));
        }
        try {
